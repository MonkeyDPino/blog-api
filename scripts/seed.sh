...
INSERT INTO categories (name, description, cover_image) VALUES
  ('Technology',   'General technology topics and trends',         NULL),
  ('JavaScript',   'JS ecosystem — runtimes, tooling, patterns',   NULL),
  ('TypeScript',   'Type-safe JavaScript development',             NULL),
  ('NestJS',       'Building scalable APIs with NestJS',           NULL),
  ('React',        'Component-driven UIs with React',              NULL),
  ('Next.js',      'Full-stack React with the App Router',         NULL),
  ('DevOps',       'CI/CD, containers, cloud infrastructure',      NULL),
  ('Architecture', 'System design and software architecture',      NULL),
  ('Career',       'Growth, interviews, and developer life',       NULL),
  ('Open Source',  'Contributing to and building OSS projects',   NULL)
ON CONFLICT (name) DO NOTHING;

-- ─────────────────────────────────────────────
-- POSTS  (30 posts — 27 published, 3 drafts)
-- Distributed across 5 authors
-- ─────────────────────────────────────────────

-- admin@pinoblog.com — 7 posts
INSERT INTO posts (title, content, summary, is_draft, author_id) SELECT
...
WHERE p.title = 'Designing Scalable REST APIs'
  AND c.name IN ('Architecture', 'NestJS', 'Technology')
ON CONFLICT DO NOTHING;

INSERT INTO posts_categories (post_id, category_id)
SELECT p.id, c.id FROM posts p, categories c
WHERE p.title = 'PostgreSQL Full-Text Search with TypeORM'
  AND c.name IN ('NestJS', 'TypeScript', 'Architecture')
ON CONFLICT DO NOTHING;
...
WHERE p.title = 'Async/Await Under the Hood'
  AND c.name IN ('JavaScript', 'TypeScript')
ON CONFLICT DO NOTHING;
...
WHERE p.title = 'Writing Clean Code in JavaScript'
  AND c.name IN ('JavaScript', 'Architecture', 'Career')
ON CONFLICT DO NOTHING;

INSERT INTO posts_categories (post_id, category_id)
SELECT p.id, c.id FROM posts p, categories c
WHERE p.title = 'Monorepos with pnpm Workspaces'
  AND c.name IN ('JavaScript', 'DevOps', 'Technology')
ON CONFLICT DO NOTHING;
